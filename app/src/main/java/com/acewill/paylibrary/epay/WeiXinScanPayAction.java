		...
		try {
			...
			synchronized (resBuf) {
				WXPay.doScanPayBusiness(data, new ResultListener() {
					...
					public void onFail(ScanPayResData arg0) {
						...
						synchronized (resBuf) {
							...
							AppApplication.getRxManager()
									.post(AppConstant.WXPAY, new SelfPosPayResult(PayResultType.PAY_FAIL, PayResultMsg.PAY_FAIL, arg0
											.getErr_code_des()));
						}
					}
					...
					@Override
					public void onSuccess(ScanPayResData arg0, String arg1) {
						//						FileLog.printResult(arg0.toString(),"helloworld");
						System.out.println("onSuccess " + arg0 + " res = "
								+ arg1);
...
//							PayStatus status = new PayStatus();
							...
							//							status.setErrmsg(arg0.getErr_code_des());
							//							status.setSetPaySuccessOutTradeNo(arg0.getOut_trade_no());
							//							AppApplication.getRxManager().post(AppConstant.WXPAY, status);
							AppApplication.getRxManager()
									.post(AppConstant.WXPAY, new SelfPosPayResult(PayResultType.PAY_SUCCESS, PayResultMsg.PAY_SUCCESS, arg0
											.getOut_trade_no()));
						}
					}
	...
	public static EPayResult query(String appId, String mchID, String key,
		...
		try {
			...
			//			FileLog.printResult("result>>" + scanPayResData.getResult_code(), "wechatPayReturn");
