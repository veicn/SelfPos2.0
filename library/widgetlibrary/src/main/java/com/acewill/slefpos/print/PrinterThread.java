...
public class PrinterThread extends Thread {
	...
	public PrinterThread(Handler handler) {
		threadInitOK = false;
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
				...
				case Global.CMD_POS_STEXTOUT: {
					...
					int    align        = data.getInt(Global.INTPARA6);
	...
	public boolean connectUsb(UsbManager manager, UsbDevice device) {
		if ((null != workHandler) && (null != mLooper)) {
			Message msg = workHandler
			...
			if (null == workHandler) {
				...
				FileLog.log("Log",PrintManager.class,"init","log","连接USB异常，workHandler为空!");
				return false;
			}

			if (null == mLooper) {
				FileLog.log("Log",PrintManager.class,"init","log","连接USB异常，mLooper!");
	...
	public boolean isConnected() {
		if (bt.IsOpened() || net.IsOpened() || usb.IsOpened())
			return true;
		else
			return false;
	}
